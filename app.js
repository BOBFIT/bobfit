...
async function motraWorkoutsFromFile(file) {
  ...
  const rows = name.endsWith(".xlsx") || type.includes("spreadsheetml") ? await xlsxRowsFromFile(file) : parseMotraTextRows(await file.text());
...
function bind() {
  ...
  $("#motra-preview-button").addEventListener("click", () => {
    try {
      motraImportPreview = parseMotraRows(parseMotraTextRows($("#motra-import-text").value));
      if (!motraImportPreview.length) throw new Error("No Motra workouts found in the pasted text.");
