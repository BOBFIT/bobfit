...
function renderReconstitution() {
  ...
  const targetCallout = result.type === "peptide" && result.idealDiluentMl
    ? `<div class="calc-callout"><strong>For a ${fmt(targetUnits)} unit draw</strong><small>Add ${fmtDose(result.idealDiluentMl)}ml bac water to this powder vial so each ${fmtDose(result.doseMg)}mg dose draws ${fmt(targetUnits)} units.</small></div>`
    : "";
  const currentMix = result.type === "oil"
    ...
    ? `<small>Enter bac water added now if you want the current draw amount as well.</small>`
    : `<div class="dose-meta">
  ...
  resultEl.innerHTML = `<strong>${escapeHtml(compoundName(peptideId))}</strong>
    <div class="formula-note">${result.type === "oil" ? "For Test E, vial strength is the oil concentration in mg/ml." : "For peptide powder, vial strength is the dry powder amount before reconstitution. The mg/ml concentration below is calculated after bac water is added."}</div>
    ${targetCallout}
    ${currentMix}
    ${componentHtml}
    ${warning}`;
