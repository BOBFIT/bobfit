...
const DEFAULT_COMPOUNDS = [
  ...
  { id: "nad", name: "NAD+", type: "peptide", vialMg: 500, diluentMl: 50, info: "Fixed calculator rule: bac water ml equals vial mg divided by 10." },
  {
    id: "glow-stack",
    name: "Glow Stack",
    type: "peptide",
    vialMg: 70,
    diluentMl: 7,
    info: "Combined stack total is 70mg. Fixed rule calculates 7ml bac water so 10 units equals 1mg of total blend.",
    components: [
      { name: "GHK-Cu", mg: 50 },
      { name: "TB100", mg: 10 },
      { name: "BPC157", mg: 10 },
...
function fixedBacWaterMl(vialMg) {
  const strength = rawNum(vialMg);
  return strength ? strength / FIXED_CONCENTRATION_MG_ML : 0;
}
function calculateFixedReconstitution(peptideId, vialMg) {
  ...
  const bacWaterMl = fixedBacWaterMl(strength);
  const totalDraws = strength / FIXED_MG_PER_DRAW;
  const componentDoses = Array.isArray(compound.components) && compound.components.length
    ? compound.components.map((component) => {
      const total = compound.components.reduce((sum, item) => sum + rawNum(item.mg), 0);
      return { name: component.name, mg: FIXED_MG_PER_DRAW * (rawNum(component.mg) / total) };
    })
    : [];
  return {
    ok: true,
    ...
    strengthMg: strength,
    bacWaterMl,
    concentrationMgMl: FIXED_CONCENTRATION_MG_ML,
    drawUnits: FIXED_DRAW_UNITS,
    drawMl: FIXED_DRAW_ML,
    mgPerDraw: FIXED_MG_PER_DRAW,
    totalDraws,
    componentDoses,
  };
}
...
function syncFixedDiluent(form, compound = compoundById(form?.elements?.peptideId?.value)) {
  ...
  const water = fixedBacWaterMl(form.elements.vialMg?.value || compound.vialMg);
...
function renderReconstitution() {
  ...
  resultEl.innerHTML = `<strong>${escapeHtml(compoundName(peptideId))}</strong>
    ...
    <div class="dose-meta">
      ...
      <span class="pill">10 units = 1mg</span>
      <span class="pill">Full vial: ${fmtDose(result.totalDraws, 1)} x 10-unit draws</span>
    </div>
    <div class="formula-note">Fixed formula: bac water ml = vial mg / 10. Example: 10mg vial / 10 = 1ml bac water.</div>
    ${componentHtml}`;
}
