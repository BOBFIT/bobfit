...
function selectedSplit(preferPlanner = false) {
  ...
  if (preferPlanner) return planned;
  const selected = state.settings.selectedSplit;
  if (state.settings.selectedSplitDate === todayKey() && selected && selected !== "rest" && state.workoutTemplates[selected]) return selected;
  return planned;
}
...
function renderReconstitution() {
  ...
  const result = calculateDraw(peptideId, form.elements.vialMg.value, form.elements.diluentMl.value, form.elements.doseMg.value, form.elements.preferredUnits.value, form.elements.doseUnit.value);
  ...
  const targetUnits = rawNum(form.elements.preferredUnits.value);
  const targetCallout = result.type === "peptide" && result.idealDiluentMl
    ? `<div class="calc-callout"><strong>You want each dose to draw ${fmt(targetUnits)} syringe units</strong><small>Mix this powder vial with ${fmtDose(result.idealDiluentMl)}ml bac water. Then your ${fmtDose(result.doseMg)}mg dose should draw ${fmt(targetUnits)} units on a U-100 syringe.</small></div>`
    : "";
  const currentMix = result.type === "oil"
    ? `<div class="dose-meta">
        <span class="pill">Oil strength: ${fmtDose(result.concentrationMgMl)} mg/ml</span>
        <span class="pill">Draw: ${fmtDose(result.drawMl)} ml</span>
      </div>`
    : result.needsCurrentDiluent
    ? `<small>Enter bac water added now if you also want to see the draw amount for your current mix.</small>`
    : `<div class="dose-meta">
        <span class="pill">After mixing: ${fmtDose(result.concentrationMgMl)} mg/ml</span>
        <span class="pill">Current draw: ${fmtDose(result.drawMl)} ml</span>
        ${result.type === "peptide" ? `<span class="pill">${fmt(result.drawUnits)} U-100 units</span>` : ""}
      </div>`;
  ...
  resultEl.innerHTML = `<strong>${escapeHtml(compoundName(peptideId))}</strong>
    <div class="formula-note">${result.type === "oil" ? "For Test E, vial strength is the oil concentration in mg/ml." : "Powder in vial is what is printed on the vial before mixing. Dose is the amount you want to take once. Syringe draw is how much liquid you pull for that one dose."}</div>
    ${targetCallout}
    ${currentMix}
    ${componentHtml}
    ${warning}`;
