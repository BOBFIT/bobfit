...
function mealHistoryMacroSummaryHtml(date, meals) {
  ...
  return `<div class="history-card macro-history-card ${status.className}">
...
function renderMealHistory() {
  ...
  $("#history-list").innerHTML = meals.length
    ? `${mealHistoryMacroSummaryHtml(date, meals)}<div class="history-card"><div class="history-head"><div><strong>${dateLabel(date)}</strong><small>${meals.length} meals / ${fmt(t.calories)} kcal / ${fmt(t.protein)}p</small></div></div><div class="meal-list">${meals.map((meal) => mealCardHtml(meal)).join("")}</div></div>`
    : `${mealHistoryMacroSummaryHtml(date, meals)}<div class="empty">No meals logged for ${escapeHtml(dateLabel(date))}.</div>`;
}
...
function renderPeptideDayHistory() {
  ...
  $("#history-list").innerHTML = `${overview}${scheduledHtml}${extraHtml}`;
}
